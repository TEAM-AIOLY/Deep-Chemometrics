...
dataset= {"data_path": "data/dataset/Mango/mango_splits.mat",
...
NF  = [1,2,3,4, 5]               # Number of filters
FC  = [32,64,128,256]          # Fully connected neurons
LR  = [0.0001, 0.001, 0.005]  # Learning rates
DP  = [0.1, 0.2, 0.5]         # Dropout rates
param_variations = [
    {"KS": ks, "NF": nf, "FC": fc, "LR": lr, "DP": dp}
    for ks, nf, fc, lr, dp in itertools.product(KS, NF, FC, LR, DP)
]
...
for i,param in enumerate(paramsets):
    ...
    model= Hmar_Net(input_dims=spec_dims, mean=mean,std=std, out_dims=y_dim, n_filters=param["NF"], kernel_size=param["KS"], fc1_dims=param["FC"], dropout=param["DP"], stride=1)
