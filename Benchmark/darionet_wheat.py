...
for i,param in enumerate(paramsets):
    ...
    # model = CuiNet(spec_dims, mean = mean,std = std,dropout=param["DP"], out_dims=y_dim)
    model = Darionet(mean=mean, std=std, filter_size=param["FS"],input_dims=spec_dims, out_dims=y_dim) 
