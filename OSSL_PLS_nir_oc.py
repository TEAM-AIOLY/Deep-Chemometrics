...
if __name__ == "__main__":
    ...
    base_path =os.path.dirname(data_path)+f"/Chimio/PLS/oC/"
    ...
    for target_label in y_labels:
       ...
       # Plot RMSECV for all folds
        ...
        y_pred_final = pls.predict(X_test, N_comp_opt)  
        ccc_value = ccc(Y_test_subset, y_pred_final)
        r2_value = r2_score(Y_test_subset, y_pred_final)
        rmsep = torch.sqrt(F.mse_loss(y_pred_final, Y_test_subset, reduction='none')).mean()
        rmsep =rmsep.numpy()
