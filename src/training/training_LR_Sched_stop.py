...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            if self.optimizer.param_groups[0]["lr"] < self.initial_lr * 1e-3:
                print(f"Early stop triggered (LR below threshold) at epoch {epoch+1}")
                break

            if best_model_path is not None:
                self.best_val_metric = Utils.save_model(
                    model=self.model,
