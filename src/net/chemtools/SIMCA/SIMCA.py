...
class SIMCA(BaseEstimator, ClassifierMixin):
    ...
    def transform(self,X):
        ...
        if not hasattr(self, "_model") or self._model is None:
            raise RuntimeError("The model has not been fitted yet. "
                        "Please call 'fit(X, y)' before calling 'transform(X)'")
    ...
    def predict(self, X, y_true=None):
        ...
        if not hasattr(self, "_model") or self._model is None:
            raise RuntimeError("The model has not been fitted yet. "
                        "Please call 'fit(X, y)' before calling 'predict()'")
    ...
    def _Tlim(self,T2,n_components):
        ...
        if self.t2lim == 'perc':
            ...
            raise ValueError("Invalid t2lim. Must be one of: 'perc', 'Fdistrig', 'Fdist', 'chi2', or 'chi2pom'.")
    ...
    def _Qlim(self,Q,modelPCA,n_components):
        ...
        if self.qlim == 'perc':
            ...
            raise ValueError("Invalid qlim. Must be one of: 'perc', 'jm', 'chi2box',  or 'chi2pom'.")
        return Q_limit
    ...
    def _critic_distance(self,T2_limit,Q_limit,T2,Q, modelPCA,n_components):
        if self.type == 'sim':
            ...
            raise ValueError("Invalid type. Must be one of: 'sim', 'alt', 'ci',  or 'dd'.")
